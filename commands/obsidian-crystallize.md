---
description: Crystallize a conversation into auditable source material and source-linked durable knowledge
category: vault
triggers_en: ["crystallize this conversation", "crystallize our exchange", "crystallize what we learned"]
triggers_es: ["cristalizar esta conversación", "cristalizar nuestro intercambio", "cristalizar lo aprendido"]
triggers_pt: ["cristalizar esta conversa", "cristalizar nossa troca", "cristalizar o que aprendemos"]
triggers_zh: ["结晶本次对话", "提炼对话内容", "将对话转化为知识"]
---

Crystallize a conversation into an auditable source transcript plus source-linked durable knowledge, artifacts, and finished reusable outputs. Use the obsidian-second-brain skill. Execute `/obsidian-crystallize $ARGUMENTS`:

Before writing, choose the right command: use `/obsidian-save` for operational memory or broad vault updates; use `/obsidian-crystallize` only when conversation content should become reusable knowledge, artifacts, source-grounded notes, or finished outputs.

1. Read vault context first:
   - Read `_AGENTS.md` first if it exists in the vault root, especially the Folder Map and any command-specific rules.
...
2. Classify the conversation before writing anything:
   ...
   - **Crystallization mode** - one or more of `source-archive`, `knowledge-distill`, `output-polish`, `decision-record`, or `learning-path`.
...
3. Save the auditable source transcript:
   - Choose the source path from the vault schema and call it `source_path`:
     - Use an existing conversation-source folder from `_AGENTS.md` or `index.md` if one exists.
     - Otherwise use `raw/transcripts/YYYY-MM-DD - <file_title>.md`.
   - Treat the verbatim source blocks as immutable raw-source material, not synthesized AI-first notes. Use Source Note frontmatter from `/obsidian-ingest`: `date`, `tags: [source, transcript, conversation]`, `source_type: transcript`, `content_hash`, optional `source_title` or `source_url` if known, plus `nature`, `mode`, and `scope` when useful for retrieval.
   - Add `## For future agent` with 2-3 sentences explaining what was preserved, why it matters, and whether the transcript is full or excerpted. This heading is for retrieval, not a signal that the raw transcript is a synthesized note.
   - Include the minimum verbatim source needed to audit the distilled notes:
     ...
     - If a synthesized note will cite a turn, include that whole surrounding user or assistant turn, not a paraphrase.
     - When uncertain whether context is needed for auditability, include it.
...
4. Add source transcript block IDs:
   ...
   - Number included blocks sequentially after excerpting; omitted turns do not reserve numbers.
...
5. Save finished output only when the conversation produced one:
   ...
   - Prefer existing project, writing, knowledge, or decision locations found in `_AGENTS.md` and `index.md`.
...
6. Synthesize reusable notes:
   ...
   - Search existing notes exhaustively by aliases and nearby terms before creating a new note. Update or extend existing notes when they cover the same concept; prefer rewriting the relevant section in place over appending a duplicate section.
   - Use the vault's existing knowledge, project, decision, or domain folders. If no suitable folder exists, choose a schema-consistent location, creating one only if necessary, then update the Folder Map later.
...
7. Apply provenance rules to every synthesized note:
   - Frontmatter includes `date`, a correct `type`, `tags`, `source`, `confidence`, `verification`, and `ai-first: true`. Use `verification: conversation-stated`, `externally-verified`, or `needs-verification`.
   - `source` must point to the chosen source transcript path, for example `source: "[[<source_path>]]"`.
   - `## For future agent` is mandatory and states what the note covers, why it matters, and any scope or staleness caveat.
   ...
   - Each synthesized note must contain at least one source-block link unless it is only a daily note, log entry, index update, or Folder Map update.
   ...
   - Mark unsourced external claims as `conversation-stated` or `needs-verification`; use `externally-verified` only when the note includes the external source or URL that verifies the claim.
...
8. Create a syllabus only when warranted:
   ...
   - Include `## For future agent`, prerequisites, recommended order, module links, complexity, and key source block links.
...
9. Update vault structure:
   ...
   - If a new folder was necessary and is not already in `_AGENTS.md`'s Folder Map, add it with a short description.
   - Confirm each path-qualified source block link resolves to an existing block ID before finishing.
   - Add or refresh `## Derived notes` in the source transcript with links only to direct derived outputs: synthesized notes, artifacts, syllabi, or learning paths. Do not include daily notes, logs, indexes, or Folder Map edits there; they are structural bookkeeping. Do not alter the verbatim source blocks.

Crystallize only durable signal. Search before creating anything. Keep enough verbatim source for auditability, but do not archive noise just because it appeared in the conversation.

---

**AI-first rule:** Every synthesized note or reusable artifact created or updated by this command MUST follow `references/ai-first-rules.md` - `## For future agent` preamble, rich frontmatter (`type`, `date`, `tags`, `ai-first: true`, plus type-specific fields), recency markers per external claim, mandatory `[[wikilinks]]` for every person/project/concept referenced, sources preserved verbatim with URLs inline, and confidence levels where applicable. If that path does not resolve from your working directory, search upward for it; if you still cannot read it, say so before writing rather than producing a note that silently skips the rule. Verbatim source blocks remain immutable inside Source Notes; synthesized outputs are for future-agent retrieval, not human reading.
